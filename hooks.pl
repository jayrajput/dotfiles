...
=cut
	...
	$search_prefixes{'^(eg|elgoog|hcraes|dnif|bew|og)(| .*)$'} = 'elgoog';     # Elgoog
	$search_prefixes{'^(p|php)(| .*)$'}                     = 'php';     # Google (default)
...
=cut
    ...
    if ($url =~ 'php\.net')
    {
        #$html =~ s/<nav id=\"trick\".*?<\/nav>//g;
        #$html =~ s/<nav id=\"head-nav\".*?<\/nav>//g;
        $html =~ s/<nav.*?<\/nav>//sg;
        $html =~ s/<link.*?>//sg;
        $html =~ s/<ul.*?PHP Manual.*?<\/ul>//s;
        #$html =~ s/<div id="goto.*?<\/div>//s;
        $html =~ s/<div id="breadcrumbs".*?<\/div>.*?<\/div>.*?<\/div>//s;
        $html =~ s/<div class="page-tools">.*?<\/div>.*?<\/div>.*?<\/div>//s;
    }
